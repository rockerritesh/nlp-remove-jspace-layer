...
## 6e. Generation gallery — 8B vs 70B vs 405B, same prompt, 0 / 25 / 50% removed

To *see* the effect across scale, here is the **same prompt** — *"The theory of relativity
states that…"* — generated by all three base models on NDIF, with a contiguous middle band
made identity (full text: [`gallery.json`](figures/gallery.json)):

| removed | **8B** (32 layers) | **70B** (80 layers) | **405B** (126 layers) |
|---|---|---|---|
| **0%** | "…the laws of physics are the same for all observers moving at a constant speed…" ✓ | "…the speed of light in a vacuum is constant… 299,792 km/s…" ✓ | "…all uniform motion is relative… no privileged reference frames…" ✓ |
| **25%** | "…a four-dimensional universe… time is the fourth dimension…" ✓ | "…the same in all inertial reference frames [repeats]… A. true B. false Answer:" ⚠ artifact | "…the space-time continuum is curved… caused by the force of gravity…" ✓ |
| **50%** | "…time slows down for the person who is in motion" ✓ | "…the speed of light is constant, regardless of the observer's frame… a fundamental principle…" ✓ | "…A. a moving clock runs faster… B. slower… C. same speed" ⚠ became a quiz |

**The honest read.** On *free generation*, all three stay **grammatically fluent even at 50%
removal**, and often stay on-topic and correct. Degradation shows up as **artifacts**
(repetition, stray multiple-choice formatting, slight topic drift) rather than gibberish.

Two things to reconcile with earlier sections: (1) the dramatic *"blue collar, a blue collar"*
collapse in §6 was the **Instruct** model with the band placed over the concept region
(L6–L17) — instruction-tuned models and that specific band are more fragile than these base
models with a centered band; (2) the big KL / low top-1 in the sweeps measure **divergence
from the original model's** next-token distribution — the ablated model predicts *different*
tokens, but when left to generate freely it re-stabilises into something coherent. So "the
middle matters" is real at the distribution level, while **free-generation fluency is more
robust than the metrics alone suggest** — especially as the model gets larger.
